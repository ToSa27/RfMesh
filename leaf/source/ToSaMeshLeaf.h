...
#define PROTO_IO_D_IN_CAPSENSE					0xC3
//#define PROTO_IO_D_IN_CAPSENSE_MAIN				0xC4
#define PROTO_IO_D_IN_DS18B20					0xC5
#define PROTO_IO_D_IN_ZEROCROSS_ACTIVE_LOW		0xC6
#define PROTO_IO_D_IN_ZEROCROSS_ACTIVE_LOW_PU	0xE6
#define PROTO_IO_D_IN_ZEROCROSS_ACTIVE_HIGH		0xC7
#define PROTO_IO_D_IN_ZEROCROSS_ACTIVE_HIGH_PU	0xE7
...
#define PARAM_CONTROL_ONOFF_MI_TOGGLE_RISING	0x04
#define PARAM_CONTROL_ONOFF_MI_TOGGLE_FALLING	0x05
...
class RfMeshLeaf : public RfMeshLeafBase {
	...
	private:
		...
		uint8_t lastTimerExecMin;
		uint8_t changeFlag[MEM_FLAG_COUNT];
		void resetFlags();
		bool isFlagged(uint8_t address);
		uint8_t memData[MEM_DATA_COUNT];
		uint16_t getValue(uint8_t address);
		void setValue(uint8_t address, uint16_t value, uint8_t setFlag);
		bool validConfig;
		void setupIO();
...
//		uint8_t capSenseCount;
//		uint8_t capSenseMain;
		unsigned long capsense_next;
		uint8_t onewire_next; // MSB is state (0 = idle - next action is start reading / 1 = reading - next action is fetch result ... 6 LSBs are second (compare to current time
		uint8_t onewire_reset(uint8_t pin);
		void onewire_writebit(uint8_t pin, uint8_t data);
		void onewire_writebyte(uint8_t pin, uint8_t data);
