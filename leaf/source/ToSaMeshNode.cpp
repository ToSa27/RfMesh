...
void RfMeshNode::LostRoute(int rti) {
	appMessage am;
	am.source = addr;
	am.dest = RF_MESH_BROADCAST;
	am.pid = 0;
	am.ptype = RF_MESH_PTYPE_ARP_FAILURE;
	am.len = 2;
	am.data[0] = (uint8_t)(rt->routes[rti].dest & 0x00FF);
	am.data[1] = (uint8_t)((rt->routes[rti].dest & 0xFF00) >> 8);
	Send(am);
	rt->routes[rti].dest = RF_MESH_NOADDRESS;
}

void RfMeshNode::TickTx() {
	for (uint16_t i = 0; i < txb->capacity; i++) {
		if (txb->pending(i)) {
			uint16_t fa = txb->failed(i);
			if (fa != RF_MESH_NOADDRESS) {
				for (int rti = 0; rti < rt->capacity; rti++) {
					if ((rt->routes[rti].dest != RF_MESH_NOADDRESS) && (rt->routes[rti].hop == rti)) {
						LostRoute(rti);
					}
				}
			}
...
//		LOG4CXX_INFO(logger, log);
			...
			switch (mm.ptype) {
				...
				case RF_MESH_PTYPE_ARP_FAILURE:
					if (1 == 1) {
						uint16_t af = (mm.data[1] << 8) + mm.data[0];
						for (int rti = 0; rti < rt->capacity; rti++) {
							if ((rt->routes[rti].dest == af) && (rt->routes[rti].hop == mm.from)) {
								LostRoute(rti);
							}
						}
					}
				...
				case RF_MESH_PTYPE_TIME_BROADCAST:
					...
					Forward(mm, true, false);
					continue;
				case RF_MESH_PTYPE_DISCOVER_REQUEST:
					if ((mm.dest == addr) || (mm.dest == RF_MESH_BROADCAST)) {
						for (uint8_t i = 0; i < rt->capacity; i++) {
							if (rt->routes[i].dest != RF_MESH_NOADDRESS) {
								meshMessage mmr;
								mmr.tc = mm.tc;
								mmr.to = mm.from;
								mmr.from = addr;
								...
								mmr.dest = mm.source;
								mmr.pid = nextpid++;
								mmr.ptype = RF_MESH_PTYPE_DISCOVER_RESPONSE;
								mmr.cost = 1;
								mmr.len = 6;
								mmr.data[0] = (uint8_t)(rt->routes[i].dest & 0x00FF);
								mmr.data[1] = (uint8_t)((rt->routes[i].dest & 0xFF00) >> 8);
								mmr.data[2] = (uint8_t)(rt->routes[i].hop & 0x00FF);
								mmr.data[3] = (uint8_t)((rt->routes[i].hop & 0xFF00) >> 8);
								mmr.data[4] = rt->routes[i].tc;
								mmr.data[5] = rt->routes[i].cost;
								Send(mmr);							
							}
						}
					}
					if (mm.dest != addr)
						Forward(mm, (mm.dest == RF_MESH_BROADCAST), false);
					continue;
